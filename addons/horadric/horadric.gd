...
static func reload_structure() -> void:
	...
	t1.instances.append("first_instance")
...
static func save() -> void:
	...
	for t in structure.templates:
		print(t.name)
		var gd_file_path = managed_dir_path + "/" + t.name.to_snake_case() + ".gd"
		var f = FileAccess.open(gd_file_path, \
				FileAccess.WRITE)
		if f == null:
		...
		# this file should now contain a valid resource
		f.close()
		
		var instance_dir_path = managed_dir_path + "/" + t.name.to_snake_case() + "s"
		
		if not DirAccess.dir_exists_absolute(instance_dir_path):
			DirAccess.make_dir_absolute(instance_dir_path)
		
		var just_created_script: Script = load(gd_file_path)
		
		for iname in t.instances:
			var copy_to_save: Resource = just_created_script.new()
			ResourceSaver.save(copy_to_save, instance_dir_path + "/" + iname + ".tres")
